...
import { usePackages, useAddOns } from '@/hooks/useSupabaseData';
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {(packages || []).map((pkg, i) => {
              ...
              const addOnIds = (pkg.add_on_ids as string[] | null) || [];
              const addOns = (allAddOns || []).filter(a => addOnIds.includes(a.id));
                ...
                <Card key={pkg.id} className={`shadow-card border-0 relative overflow-hidden ${i === 1 ? 'ring-2 ring-accent shadow-elevated' : ''}`}>
                  ...
                  <CardContent className="pt-8 pb-6">
                    ...
                    <div className="bg-secondary rounded-lg p-4 mb-5">
                      <div className="flex justify-between text-sm mb-1"><span className="text-muted-foreground">Per Game</span><span className="font-semibold">${pkg.per_game_usd}</span></div>
                      <div className="flex justify-between text-sm mb-1"><span className="text-muted-foreground">Full Day</span><span className="font-semibold">${pkg.per_day_usd}</span></div>
                    ...
                    <ul className="space-y-2 mb-5">
                      {features.map((f, fi) => (
                        <li key={fi} className="flex items-start gap-2 text-sm text-foreground"><Star className="h-4 w-4 text-accent mt-0.5 shrink-0" />{f}</li>
                      ))}
                    </ul>
                    {addOns.length > 0 && (
                      <div className="mb-5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {addOns.map(a => <span key={a.id} className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{a.name}</span>)}
                        </div>
                      </div>
                    )}
