...
import { MessageSquare, Send, FileText, Plus, Pencil, Trash2 } from 'lucide-react';
...
export default function AdminSms() {
  ...
  const handleBookingSelect = (bookingId: string) => {
    ...
    if (bookingId !== 'none') {
      ...
      // Re-apply template with new booking context
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['{{name}}', '{{team}}', '{{date}}', '{{event}}', '{{park}}', '{{phone}}', '{{email}}'].map(v => (
                      <div key={v} className="flex items-center gap-2 rounded-md bg-muted px-3 py-2 text-sm font-mono">
                        {v}
                      </div>
                    ))}
                  </div>
