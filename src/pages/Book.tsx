...
import {
  mockEvents, mockPackages, mockParks, mockFields, mockSpots,
  mockAddOns, takenSpotIds, formatCurrency, getPackagePrice, getAddOnPrice
} from '@/data/mockData';
...
export default function Book() {
  ...
  const update = (partial: Partial<BookingFormData>) => setForm(prev => ({ ...prev, ...partial }));

  // Derived data
  const selectedEvent = mockEvents.find(e => e.id === form.eventId);
  const selectedPkg = mockPackages.find(p => p.id === form.packageId);
  const availableParks = selectedEvent ? mockParks.filter(p => selectedEvent.parkIds.includes(p.id)) : mockParks;
  const selectedPark = mockParks.find(p => p.id === form.parkId);
  const availableFields = mockFields.filter(f => f.parkId === form.parkId);
  const selectedField = mockFields.find(f => f.id === form.fieldId);
  const availableSpots = mockSpots.filter(s => s.fieldId === form.fieldId);
  const availableAddOns = selectedPkg ? mockAddOns.filter(a => selectedPkg.addOnIds.includes(a.id)) : [];

  // Pricing
  const baseCents = selectedPkg ? getPackagePrice(selectedPkg, form.packageMode) : 0;
  const addOnCents = form.addOns.reduce((sum, ao) => {
    const addOn = mockAddOns.find(a => a.id === ao.addOnId);
    return sum + (addOn ? getAddOnPrice(addOn, form.packageMode) * ao.qty : 0);
  }, 0);
  ...
  const getAddOnQty = (addOnId: string) => form.addOns.find(a => a.addOnId === addOnId)?.qty || 0;
  const setAddOnQty = (addOnId: string, qty: number) => {
    if (qty <= 0) {
      update({ addOns: form.addOns.filter(a => a.addOnId !== addOnId) });
    } else {
      const exists = form.addOns.find(a => a.addOnId === addOnId);
      if (exists) {
        update({ addOns: form.addOns.map(a => a.addOnId === addOnId ? { ...a, qty } : a) });
      } else {
        update({ addOns: [...form.addOns, { addOnId, qty }] });
      }
    }
  };

  const applyDiscount = () => {
    if (form.discountCode.toUpperCase() === 'SIDELINE10') {
      setDiscountSavings(Math.round(subtotal * 0.1));
  ...
  return (
    <PublicLayout>
      ...
      <div className="container py-8 max-w-3xl">
        ...
        {/* Step 1: Select Event (optional pre-select) */}
        {!form.eventId && (
          <section className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {mockEvents.filter(e => e.isActive).map(event => (
                <button
                  key={event.id}
                  onClick={() => update({ eventId: event.id, parkId: '', fieldId: '', spotId: '' })}
                  className={`text-left p-4 rounded-lg border-2 transition-all ${
                    form.eventId === event.id ? 'border-accent bg-accent/5' : 'border-border hover:border-accent/50'
                  }`}
                >
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {mockPackages.filter(p => p.isActive).map(pkg => (
              <button
                key={pkg.id}
                onClick={() => update({ packageId: pkg.id, addOns: [] })}
                className={`text-left p-4 rounded-lg border-2 transition-all ${
                  form.packageId === pkg.id ? 'border-accent bg-accent/5 shadow-card' : 'border-border hover:border-accent/50'
                }`}
              >
                ...
                <p className="text-sm font-semibold text-accent">From ${pkg.perGameUsd}/game</p>
              </button>
            ))}
          </div>
        </section>
          ...
          <section className="mb-10 animate-fade-in-up">
            ...
            <div className="flex flex-wrap gap-3">
              {(['per_game', 'day', 'weekend'] as const).map(mode => (
                <button
                  key={mode}
                  onClick={() => update({ packageMode: mode })}
                  className={`px-5 py-3 rounded-lg border-2 font-heading font-semibold text-sm transition-all ${
                    form.packageMode === mode
                ...
                >
                  ...
                  {selectedPkg && (
                    <span className="block text-xs mt-0.5">
                      {formatCurrency(getPackagePrice(selectedPkg, mode))}
                    </span>
                  )}
                </button>
              ))}
                  ...
                  <Card key={addOn.id} className={`border-2 ${qty > 0 ? 'border-accent' : 'border-border'}`}>
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-sm font-semibold text-accent whitespace-nowrap ml-2">
                          +{formatCurrency(getAddOnPrice(addOn, form.packageMode))}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 mt-3">
                        <Button size="icon" variant="outline" className="h-8 w-8" onClick={() => setAddOnQty(addOn.id, qty - 1)} disabled={qty === 0}>
                          <Minus className="h-4 w-4" />
                        </Button>
                        <span className="font-bold text-foreground w-6 text-center">{qty}</span>
                        <Button size="icon" variant="outline" className="h-8 w-8" onClick={() => setAddOnQty(addOn.id, qty + 1)}>
                ...
                {form.gameTimes.map((gt, i) => (
                  <div key={i} className="flex gap-2 mt-1">
                    <Input
                      ...
                      value={gt}
                      onChange={e => {
                        const newTimes = [...form.gameTimes];
                        newTimes[i] = e.target.value;
                        update({ gameTimes: newTimes });
                      }}
                    />
                    {form.gameTimes.length > 1 && (
                      <Button variant="ghost" size="icon" onClick={() => update({ gameTimes: form.gameTimes.filter((_, j) => j !== i) })}>
                        <Minus className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
                ))}
                <Button variant="ghost" size="sm" className="mt-2" onClick={() => update({ gameTimes: [...form.gameTimes, ''] })}>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-3">
                {availableParks.map(park => (
                  <button
                    key={park.id}
                    onClick={() => update({ parkId: park.id, fieldId: '', spotId: '' })}
                    className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                      form.parkId === park.id ? 'border-accent bg-accent/5' : 'border-border hover:border-accent/50'
                    }`}
                  >
                    {park.name}
                  </button>
              ...
              <div className="mb-4 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-3">
                  {availableFields.map(field => (
                    <button
                      key={field.id}
                      onClick={() => update({ fieldId: field.id, spotId: '' })}
                      className={`px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                        form.fieldId === field.id ? 'border-accent bg-accent/5' : 'border-border hover:border-accent/50'
                      }`}
                    >
                      {field.name}
                    </button>
          ...
          <section className="mb-10 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg">
              <div className="sm:col-span-2">
                ...
                <Input value={form.fullName} onChange={e => update({ fullName: e.target.value })} placeholder="Jane Smith" />
              </div>
              <div>
                <Label>Email *</Label>
              ...
              <div>
                <Label>Phone *</Label>
                <Input type="tel" value={form.phone} onChange={e => update({ phone: e.target.value })} placeholder="(903) 555-1234" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={form.coachName} onChange={e => update({ coachName: e.target.value })} />
              </div>
              <div className="sm:col-span-2">
          ...
          <section className="mb-10 animate-fade-in-up">
            ...
            <div className="space-y-4 max-w-lg">
              ...
              <div className="flex items-start gap-3">
                ...
                />
                <label htmlFor="sms" className="text-sm text-muted-foreground leading-snug cursor-pointer">
                  I consent to receive SMS notifications about my booking. Message and data rates may apply. Reply STOP to unsubscribe.
                </label>
              </div>
            </div>
          </section>
          ...
          <section className="mb-10 animate-fade-in-up">
            ...
            <div className="flex gap-2 max-w-sm">
              <Input
                ...
                onChange={e => { update({ discountCode: e.target.value }); setDiscountApplied(false); setDiscountSavings(0); }}
            ...
            {discountApplied && (
              <p className="text-sm text-success font-semibold mt-2">
                ✓ Code applied! You save {formatCurrency(discountSavings)}
              </p>
            )}
            <p className="text-xs text-muted-foreground mt-1">Try: SIDELINE10</p>
          </section>
        )}
                ...
                <table className="w-full text-sm">
                    ...
                    {form.addOns.map(ao => {
                      ...
                      return (
                        <tr key={ao.addOnId} className="border-b border-border">
                          ...
                          <td className="py-2 text-right">{formatCurrency(getAddOnPrice(addOn, form.packageMode) * ao.qty)}</td>
                        </tr>
                      );
                    })}
                    {discountSavings > 0 && (
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td className="pt-4 text-right font-heading font-bold text-lg text-accent">{formatCurrency(totalCents)}</td>
                    </tr>
                  </tfoot>
                </table>
              </CardContent>
          ...
          <section className="mb-10 animate-fade-in-up">
            ...
            <Card className="border-2 border-border bg-secondary">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-muted-foreground mb-4">PayPal payment buttons will appear here when connected to your backend.</p>
                <Button className="bg-gradient-cta text-primary-foreground shadow-glow-amber font-heading font-semibold">
      ...
      {step1Done && (
        <div className="fixed bottom-0 left-0 right-0 bg-primary text-primary-foreground border-t border-primary z-50">
          <div className="container flex items-center justify-between py-3">
            ...
            {selectedPkg && (
              <Badge className="bg-accent text-accent-foreground font-heading">{selectedPkg.name}</Badge>
            )}
          </div>
        </div>
      )}
